...
class Sidebar(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        # Ajouter un menu dÃ©roulant pour sÃ©lectionner un dossier de blueprints
    ...
    def update_game_folder(self, selected_folder):
        """Met Ã  jour le dossier de blueprints sÃ©lectionnÃ©."""
        chemin_base = from core.paths import get_blueprints_base
    ...
    def update_blueprints(self):
        """Met Ã  jour la liste des blueprints dans la fenÃªtre principale."""
...
class App(ctk.CTk):
    def __init__(self):
        ...
        if os.path.exists(icon_path):
            ...
            print("IcÃ´ne introuvable")

        # Forcer l'icÃ´ne dans la barre des tÃ¢ches (uniquement sous Windows)
        if os.name == "nt":  # VÃ©rifie que le systÃ¨me est Windows
            app_id = "Satisfactory blueprint Manager"  # Identifiant unique pour votre application
            ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(app_id)
            self.iconbitmap(icon_path)  # Appliquer l'icÃ´ne

        # Chemin attendu pour le dossier blueprints
        chemin_blueprints = from core.paths import get_blueprints_base
    ...
    # DÃ©finir la mÃ©thode avant l'utilisation
    def scroll_scim_window(self, event):
        """GÃ¨re le dÃ©filement dans la fenÃªtre SCIM."""
        try:
            if event.delta < 0:
            ...
            print(f"Erreur lors du dÃ©filement dans la fenÃªtre SCIM : {e}")
    ...
    def open_scim_button_callback(self):
        ...
        # Lier la molette de la souris au canvas dans la fenÃªtre SCIM
        blueprint_window.bind("<MouseWheel>", self.scroll_scim_window)

        # Cadre de navigation pour la pagination (inchangÃ©)
        ...
        # Afficher un message "TÃ©lÃ©chargement en cours"
    ...
    def bind_scim_mousewheel(self, blueprint_window):
        """Lier la molette de la souris au dÃ©filement dans la fenÃªtre SCIM."""
        blueprint_window.bind_all("<MouseWheel>", self.scroll_scim_window)

    def scroll_main_window(self, event):
        """GÃ¨re le dÃ©filement dans la fenÃªtre principale."""
        try:
            if event.delta < 0:
            ...
            print(f"Erreur lors du dÃ©filement de la fenÃªtre principale : {e}")

    # ON PASSE SUR LE LOAD BP DE SCIM

    def load_scim_blueprints(self, site_page):
        """Charge les blueprints depuis la page spÃ©cifiÃ©e du site"""
        # Efface les Ã©lÃ©ments prÃ©cÃ©dents dans le cadre
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()

        # Mettre Ã  jour le label de numÃ©ro de page
        self.page_label.configure(text=f"Page {site_page}")

        # On remonte en haut de la fenetre pour afficher les BP aprÃ¨s un changement de page
        ...
        # RÃ©cupÃ©ration des donnÃ©es des blueprints
        ...
        for item in blueprint_items:
            # RÃ©cupÃ©ration du lien, de l'image, du titre et de l'ID du blueprint
            link = item.find("a", href=True)
            if link is None:
                continue

            # RÃ©cupÃ©ration du titre dans <h6><strong>
            h6_tag = item.find("h6")
            title_tag = h6_tag.find("strong") if h6_tag else None
            title = title_tag.get_text(strip=True) if title_tag else "Sans Titre"

            # RÃ©cupÃ©ration de l'ID dans le href
            href = link.get("href", "")
            blueprint_id = href.split("/id/")[1].split("/")[0] if "/id/" in href else None
            if not blueprint_id:
                continue

            # RÃ©cupÃ©ration de l'URL de l'image dans <img>
            image_tag = link.find("img")
            image_url = image_tag["src"] if image_tag and "src" in image_tag.attrs else None
            if not image_url:
                continue

            # RÃ©cupÃ©rer la description courte du blueprint
            ...
            try:
                ...
                print(f"Erreur lors du tÃ©lÃ©chargement de l'image : {e}")
                continue
    ...
    def download_blueprint(self, blueprint_id, title):
        """TÃ©lÃ©charge les fichiers .sbp et .sbpcfg pour un blueprint sÃ©lectionnÃ©"""
        ...
        # On nettoie l'url SCIM si caractÃ¨res bizarre
        ...
        try:
            ...
            if os.path.exists(sbp_file_path) and os.path.exists(sbpcfg_file_path):
                ...
                return  # Ne pas procÃ©der au tÃ©lÃ©chargement si le BP existe dÃ©jÃ 

            # TÃ©lÃ©chargement des fichiers si non existants
            ...
            if sbp_response.status_code == 200 and sbpcfg_response.status_code == 200:
                # Sauvegarder les fichiers tÃ©lÃ©chargÃ©s dans le repertoire windows
    ...
    def prev_site_page(self):
        """Affiche la page prÃ©cÃ©dente de blueprints sur le site"""
        if self.current_site_page > 1:
            self.current_site_page -= 1
            self.update_page()

    def update_page(self):
        """Met Ã  jour la page des blueprints en utilisant un thread"""
        # Effacer le cadre et afficher un message "TÃ©lÃ©chargement en cours"
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()
        self.loading_label = ctk.CTkLabel(self.scrollable_frame, text="TÃ©lÃ©chargement en cours...")
        self.loading_label.pack(pady=20)
    ...
    def get_blueprint_description(self, blueprint_id):
        """RÃ©cupÃ¨re et retourne une courte description (150 mots) du blueprint"""
        ...
        # 1. Rechercher dans des blockquotes indÃ©pendants
        ...
        # Extraire et limiter la description si trouvÃ©e
        if description_tag:
            description = description_tag.get_text(strip=True)
            short_description = " ".join(description.split()[:150]) + "..."
        else:
            # Si rien n'est trouvÃ©, message pour aider Ã  l'inspection
            print("Aucune description trouvÃ©e")  # Limite d'affichage Ã  1000 caractÃ¨res
            short_description = self.i18n.t('scim_no_description')

        return short_description

    def select_blueprint_folder(self, default_path):
        """Affiche une boÃ®te de dialogue pour sÃ©lectionner un dossier."""
    ...
    def show_about(self):
        """Affiche une boÃ®te de dialogue Ã€ propos."""
        messagebox.showinfo(self.i18n.t('menu_about'), self.i18n.t('software_aboutsbm'))

    def show_help(self):

        """Affiche une nouvelle fenÃªtre avec du texte formatÃ© pour expliquer le fonctionnement."""
    ...
    def display_update_status(self):
        ...
        """VÃ©rifie les mises Ã  jour disponibles et affiche des messages en consÃ©quence."""
        is_up_to_date, remote_version, download_url, error_message = self.backend.check_for_update(BUILD_NUMBER)

        if error_message:
            # En cas d'erreur rÃ©seau ou autre, afficher un message d'erreur
            messagebox.showerror(self.i18n.t("error"), error_message)
        elif not is_up_to_date:
            # Mise Ã  jour disponible, afficher l'information
            messagebox.showinfo(
            ...
            # Si aucune mise Ã  jour n'est nÃ©cessaire, continuer normalement
            return True
